...
def apply_normalize(crops):
    mean_pix = [x/255.0 for x in [86.98044165811156, 87.06433144815632, 86.97865571565083]]
    std_pix = [x/255.0 for x in [38.215870650605936, 38.20272750914371, 38.21268153184896]]
    return torch.stack([T.Normalize(mean=mean_pix, std=std_pix)(crop) for crop in crops])

class infraredWCS(Dataset):
    """
...
"""
    def __init__(self, root, resize, split, mode, augment, img_size=224):
        ...
        wcs_path = './dataset/wcs/dataset/infrared_dataset/'
        cct_path = "./dataset/cct/dataset/infrared_dataset/"
        ...
        train_train_sample_list = torch.load('./dataset/wcs/dataset/infrared_train_sample_list.pt')
        # train_val_sample_list = torch.load('D:\GraduationProject\Dataset\WCS_Camera_Traps\dataset\infrared_train_sample_list.pt')
        # train_test_sample_list = torch.load('D:\GraduationProject\Dataset\WCS_Camera_Traps\dataset\infrared_train_sample_list.pt')
        val_sample_list = torch.load('./dataset/wcs/dataset/infrared_test_sample_list.pt')
        test_sample_list = torch.load('./dataset/wcs/dataset/infrared_test_sample_list.pt')
    ...
    def __getitem__(self, index):
        if self.mode == 'regular':
            ...
            # indexがclosed-setのサンプルの数よりも小さい場合、普通のデータを返す。
