...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        # テンソルとして読み込む(nn.Parameterにすることで勾配計算が行われる）
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        self.pos_drop = nn.Dropout(p=drop_rate)
        # 確率的深さというテクニック
        # 深さをランダムに削減し、ロバスト性と安定性を上げる。
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        # パッチごとに行なう処理であるBlockを実装。
        ...
        # Classifier head
        # 出力層を定義している。
        # 全結合層を表し、embed_dimからクラス数の次元への線形変換を行なう。
        # クラス数が０以下の場合、出力は何も変換されずにそのままになる。
        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        # 二ューラルネットワークモデルの重みやバイアスを特定の範囲内で初期化する際に使用され、
        # 特に初期化方法に制約を加えたい場合に役立つ
